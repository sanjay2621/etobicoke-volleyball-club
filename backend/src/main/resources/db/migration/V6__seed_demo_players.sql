...
-- Get existing tournament or create one
SET @tid = 0;
SELECT COALESCE(MIN(id), 0) INTO @tid FROM tournament WHERE deleted = 0;
...
FROM DUAL WHERE @tid = 0;

SET @tid = IF(@tid = 0, LAST_INSERT_ID(), @tid);
